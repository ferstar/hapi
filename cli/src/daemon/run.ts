    ...
    try {
        ...
        const spawnSession = async (options: SpawnSessionOptions): Promise<SpawnSessionResult> => {
            ...
            if (sessionType === 'worktree') {
                ...
                logger.debug(
                    `[DAEMON RUN] Created worktree ${worktreeInfo.worktreePath} (branch ${worktreeInfo.branch})`
                )
            }
            ...
            try {
                ...
                const agentCommand = agent === 'codex' ? 'codex' : agent === 'gemini' ? 'gemini' : 'claude'
        ...
        const machine = await withRetry(
                ...
                }),
            {
                ...
                onRetry: (error, attempt, nextDelayMs) => {
                    ...
                    logger.debug(
                        `[DAEMON RUN] Failed to register machine (attempt ${attempt}), retrying in ${nextDelayMs}ms: ${errorMsg}`
                    )
                },
            }
        )
